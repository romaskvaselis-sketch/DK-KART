...
function SettingsView({ profile, onSave, onReset, onLoadBaseline, onExport, onImport, sessionCount }) {
  ...
  return (
    <div>
      ...
      {onLoadBaseline && (
        <button 
          style={{ ...styles.btnGhost, borderColor: C.accent, color: C.accent }} 
          onClick={onLoadBaseline}
        >
          📥 Užkrauti baseline sesijas (Dovydo praeities duomenys)
        </button>
      )}
...
export default function DKKart() {
  ...
  // Užkrauti baseline sesijas (Dovydo praeitų savaitgalių duomenis)
  // Naudinga, jei sesijos dingo po atnaujinimo arba reikia atstatyti
  const handleLoadBaseline = async () => {
    try {
      // Patikrinti, ar naudojamas savininko invite kodas
      const inviteR = await window.storage.get(INVITE_CODE_KEY).catch(() => null);
      if (!inviteR || inviteR.value !== OWNER_INVITE_CODE) {
        alert("⚠ Baseline sesijos galimos tik su savininko invite kodu (DK-DOVYDAS-2026)");
        return;
      }
      
      // Patikrinti, ar jau yra sesijų — jei taip, paklausti
      if (sessions.length > 0) {
        const overwrite = window.confirm(
          `⚠ Yra ${sessions.length} esamų sesijų.\n\n` +
          `Norite PRIDĖTI baseline sesijas prie esamų (gali atsirasti dublikatų), ar PERRAŠYTI viską?\n\n` +
          `OK = PRIDĖTI prie esamų\n` +
          `Cancel = nutraukti\n\n` +
          `(Jei norite PERRAŠYTI viską — pirma rankiniu būdu ištrinkite sesijas)`
        );
        if (!overwrite) return;
        
        // Pridėti, vengiant dublikatų pagal ID
        const existingIds = new Set(sessions.map(s => s.id));
        const newSessions = DOVYDAS_BASELINE_SESSIONS.filter(s => !existingIds.has(s.id));
        if (newSessions.length === 0) {
          alert("ℹ Visos baseline sesijos jau yra įvestos.");
          return;
        }
        const merged = [...sessions, ...newSessions];
        await window.storage.set(getSessionsKey(activeDriverId), JSON.stringify(merged));
        setSessions(merged);
        alert(`✓ Pridėta ${newSessions.length} naujų baseline sesijų. Iš viso: ${merged.length}`);
      } else {
        // Tuščia — tiesiog užkrauti
        await window.storage.set(getSessionsKey(activeDriverId), JSON.stringify(DOVYDAS_BASELINE_SESSIONS));
        setSessions(DOVYDAS_BASELINE_SESSIONS);
        alert(`✓ Užkrauta ${DOVYDAS_BASELINE_SESSIONS.length} baseline sesijų`);
      }
    } catch (e) {
